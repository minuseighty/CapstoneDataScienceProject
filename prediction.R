...
save(pred.bi, file = "predbi.RData")
rm(bi.df, bi.split)

tri.df$word <- as.character(tri.df$word)
tri.split <- data.frame(t(data.frame(str_split(tri.df$word, ' '))))
...
save(pred.tri, file = "predtri.RData")
rm(tri.df, tri.split)
...
save(pred.quad, file = "predquad.RData")
